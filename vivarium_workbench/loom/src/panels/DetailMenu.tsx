...
export default function DetailMenu(props: {
  ...
  const anyForced = overrides.ports !== 'auto' || overrides.config !== 'auto' || overrides.contract !== 'auto';
    ...
    <div style={{ display: 'inline-flex', border: '1px solid #d1d5db', borderRadius: 5, overflow: 'hidden' }}>
      {opts.map((o, i) => (
        <button
          ...
          onClick={() => onPick(o.id)}
          style={{
            padding: '3px 9px', fontSize: 11, border: 0, cursor: 'pointer',
  ...
  const rowLabel: React.CSSProperties = { fontSize: 11.5, color: '#374151', width: 62, flexShrink: 0 };
  const row: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: 10, padding: '6px 12px' };

  return (
    <div ref={wrapRef} style={{ position: 'relative' }}>
      <button
        onClick={() => setOpen((v) => !v)}
        title="Card detail — toggle ports / config / contract independently, or follow the zoom (Auto)"
        style={{
      ...
      {open && (
        <div style={{
          ...
          boxShadow: '0 2px 10px rgba(0,0,0,0.14)', overflow: 'hidden', zIndex: 20, minWidth: 244,
        }}>
          ...
          <div style={row}><span style={rowLabel}>Contract</span>{seg(TRI, overrides.contract, (id) => setOverrides({ ...overrides, contract: id as TriDetail }))}</div>
          <div style={{ height: 1, background: '#e5e7eb', margin: '4px 0' }} />
          <button
            onClick={() => setOverrides({ ports: 'auto', config: 'auto', contract: 'auto' })}
            disabled={!anyForced}
            style={{

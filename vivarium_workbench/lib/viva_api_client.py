...
class VivaApiClient:
    ...
    def _get(self, path: str, params: dict | None = None) -> dict:
        ...
        try:
            ...
            raise VivaApiError(f"GET {url} -> {e.code}") from e
        except (URLError, OSError) as e:
            raise VivaApiError(f"GET {url} failed (sms-api unreachable — is the tunnel up?): {e}") from e
    ...
    def ping(self, timeout: float | None = None) -> str:
        ...
        Returns the sms-api version string; raises :class:`VivaApiError` if the
    ...
    def composite_resolve(self, simulator_id: int, composite_ref: str,
        ...
        dashboard's local /api/composite-resolve). Raises VivaApiError on failure.
        """
        return self._post(
    ...
    def _post(self, path: str, params: dict | None = None, json_body: dict | None = None) -> dict:
        ...
        try:
            ...
            raise VivaApiError(f"POST {url} -> {e.code}") from e
        except (URLError, OSError) as e:
            raise VivaApiError(f"POST {url} failed (sms-api unreachable — is the tunnel up?): {e}") from e
    ...
    def compose_check(self, pbg_bytes: bytes) -> dict:
        ...
        Raises :exc:`VivaApiError` if the server is unreachable or returns a
        non-200 status.
        """

...
def build_investigation_document(ws_root: Path | str, inv_slug: str) -> dict:
    ...
    # importable (e.g. for build_investigation_composite below) without
    # requiring process_bigraph, matching workspace_commit/node_cache_status's
    # local-import pattern in this same file.
...
# ---------------------------------------------------------------------------
# InvestigationComposite generator (investigation-as-composite, Task 5) ------
#
# Distinct from ``build_investigation_document`` above (the Layer-4
# pull-or-compute *trigger* document). This builds a real process-bigraph
# composite **state dict** — one ``StudyStep`` per member study, wired by
# ``pipeline_gate.prerequisites`` (+ synthetic serial edges for declared-order
# determinism), plus one ``InvestigationAnalysisStep`` per ``analyses:``
# entry — that ``Composite({"state": ..., "run_steps_on_init": True}, core)``
# actually runs to execute the investigation.
#
# Design: docs/superpowers/specs/2026-08-01-investigation-as-composite-design.md
# §Architecture 3. Step-doc shape (store nodes are empty dicts ``{}``, plain
# ``"local:StudyStep"``/``"local:InvestigationAnalysisStep"`` addresses) is the
# shape validated against the real engine in
# ``tests/test_investigation_steps_skeleton.py`` /
# ``tests/test_investigation_analysis_step.py`` (Tasks 1 + 4).
# ---------------------------------------------------------------------------


def _normalize_member_slug(entry: Any) -> str | None:
    """A ``studies:``/``members:`` entry may be a bare slug string or a dict
    (``{study|slug|name: ...}``, per ``investigation_member_slugs``'s
    docstring) — normalize to the slug string, or ``None`` if unresolvable."""
...
def _study_prereqs(ws: WorkspacePaths, slug: str) -> list[str]:
    ...
    ``pipeline_gate.prerequisites`` — NOT the legacy ``parent_studies``
    fallback. Mirrors ``prepare_investigation._study_prereqs`` (#712,
    5126f15b): each entry is a dict ``{study: X, ...}`` or a bare string
    ``X``. Keying strictly on this field makes reading a no-``pipeline_gate``
    study.yaml a no-op (empty list) rather than silently picking up legacy
    ``parent_studies`` edges."""
...
def build_investigation_composite(ws_root: Path | str, inv_slug: str) -> dict:
    """Compile ``investigations/<inv_slug>/investigation.yaml`` into a
    composite state dict: one ``StudyStep`` per member study (prerequisite-
    ordered, real or synthetic-serial) + one ``InvestigationAnalysisStep`` per
    ``analyses:`` entry wired to every member study's result store.

    Ordering has two mechanisms, both expressed purely as input wires (no
    explicit scheduling code — ``process_bigraph``'s ``determine_steps`` reads
    the wires):

    1. **Real prerequisites** — a study's ``pipeline_gate.prerequisites``,
       filtered to entries that are members of THIS investigation.
    2. **Synthetic serial edges** — confirmed necessary in Task 1: independent
       steps (no wiring between them) run in NONDETERMINISTIC engine order,
       not declared order. A study with no real prerequisite is wired instead
       to the immediately-preceding declared member, turning a whole
       no-prerequisite investigation into a deterministic declared-order
       chain. (v1 is serial; dropping these edges for genuinely-independent
       studies is a later parallelism pass.)

    Returns a plain state dict (NOT a running ``Composite``) — callers wrap it
    as ``Composite({"state": build_investigation_composite(...),
    ...
    for entry in investigation_member_slugs(spec):
        slug = _normalize_member_slug(entry)
        if slug:
            members.append(slug)
    ...
    for i, slug in enumerate(members):
        ...
        if real_prereqs:
            ...
            # immediately-preceding declared member (declared-order
            # determinism — see function docstring).
            prereqs = [members[i - 1]]
        else:
            prereqs = []

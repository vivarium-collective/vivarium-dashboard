...
def composite_test_run(ws_root: Path, body: dict) -> tuple[dict, int]:
    ...
    # B (demo blocker) / item 18: when this workbench is configured for remote
    # runs (VIVARIUM_WORKBENCH_REMOTE_PINNED — the operator's declaration that
    # it dispatches to an sms-api deployment), route the Composites-tab Run to
    # the 'deployment' target (compose-on-Batch via remote_run.run_remote)
    # instead of spawning the full model in this lightweight pod. The prod
    # workspace has no `.viv-build.json`, so run_core.run_target_for alone
    # would otherwise resolve to 'local' — resolve_run_target is the ONE
    # function every run entrypoint (this one AND study_runs.py) calls so

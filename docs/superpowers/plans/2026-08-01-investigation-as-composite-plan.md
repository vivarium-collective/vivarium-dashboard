...
**Interfaces:**
- Produces: `build_investigation_composite(ws_root, inv_slug) -> dict` (a composite **state dict**, not a running Composite): one `StudyStep` doc per member study (config incl. `prereqs`), each study's `result` store (an empty dict `{}`), prereq input wires, plus one `AnalysisStep` per `investigation.yaml` `analyses:` entry wired to the relevant studies' result stores. Reuses `investigation_member_slugs` + the strict `pipeline_gate.prerequisites` read (carried from #712's `_study_prereqs`).
- **Deterministic declared order (confirmed necessary in Task 1):** independent studies run in nondeterministic engine hash order. So for each study with NO real `pipeline_gate.prerequisites`, add a **synthetic serial edge** to the immediately-preceding declared member (wire its `prereq_<prev>` input to `<prev>`'s result store), producing a deterministic declared-order chain. A study WITH real prerequisites uses those instead. (v1 is serial; a later parallelism pass drops synthetic edges for genuinely-independent studies.) Store nodes are `{}`; port type is `"node"`; step-doc shape and `allocate_core(top=...)` registration are as validated in Task 1.

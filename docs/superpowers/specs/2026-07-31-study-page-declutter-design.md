...
| **Exports** | Download artifacts | Any explanatory prose | Keep the analysis-results list + "Download all (.zip)" + raw-simulation-data list. Gains the **Analyses** config (relocated from Model, Increment 2), co-located with the analysis outputs it produces. |

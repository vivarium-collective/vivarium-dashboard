...
**Status:** proposed, 2026-08-27. Follows §2A.8 workstream 8 steps 1 and 2a
(#952, #954, #957), which are built and deployed to `sms-api-stanford-test`.
...
| `/api/investigation-run` | inline `textwrap` script + `subprocess` | **no — zero references** |
...
### A1 — Close the `/api/investigation-run` bypass *(live defect)*

Fix it **at the executor seam, not by re-pointing the route.**
...
So: inside `run_one_composite`, resolve the target and, on `deployment`, dispatch
through the path `study_runs` already uses (`run_core.invoke_run` →
`run_runner`) rather than building an embedded subprocess script.

Re-pointing the whole route at `run_jobs` was considered and rejected: it would
mean re-implementing everything `run_investigation` does — the multi-composite
`state_doc` path (emitter step injected by `inject_emitter_step`), the viz hooks
(`viz_render_hooks`), spec-error handling, and the concurrent run-lock guard that
must surface as 404 — all while fixing a live defect. The route contract is
unchanged.
...
1. **A1** — live defect; one closure, no contract change.
2. **A4** — verify dispatch end-to-end (no code).
3. **A2** — durability, before concurrency multiplies what a restart can lose.
4. **A3** — concurrency, then prereq ordering.
5. **A5** — converge, once A2/A3 make `run_jobs` a superset.

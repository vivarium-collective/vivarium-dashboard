...
(function () {
  ...
  function _feedbackWidgetJs(invName) {
    return '<script>'
      ...
      +   'var openEd=null;'  // currently-open editor (singleton)
      ...
      +         '<input class=\\"fb-author\\" placeholder=\\"Your name (optional)\\" value=\\""+esc(localStorage.getItem("fb_author")||"")+"\\">'
      ...
      +       'if(author)localStorage.setItem("fb_author",author);'
      ...
      +   'function updateBarCount(){var c=countAll();var s=document.querySelector(".fb-count");if(s)s.textContent="("+c+")";var btn=document.querySelector(".fb-bar-btn");if(btn)btn.disabled=c===0;}'
      +   'function ensureBar(){'
      +     'if(document.querySelector(".fb-bar"))return;'
      +     'var bar=document.createElement("div");bar.className="fb-bar";'
      +     'bar.innerHTML="<button type=\\"button\\" class=\\"fb-bar-btn\\" title=\\"Download all your annotations as a yaml file; send it back to the investigation\\">Generate feedback report <span class=\\"fb-count\\">(0)</span></button>";'
      +     'document.body.appendChild(bar);'
      +     'bar.querySelector(".fb-bar-btn").addEventListener("click",function(e){e.preventDefault();e.stopPropagation();downloadFeedback();});'
      +   '}'
